...
select
  ...
  player_index,
  (case when winner then pending_cards || retained_cards else retained_cards end) as packet
...
where
  1 < (
    ...
    select count(distinct(player_index, packet))
    from last_simulation_hand_packets
    where array_length(packet, 1) > 0
  );
